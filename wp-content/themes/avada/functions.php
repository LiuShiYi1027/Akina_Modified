...
function get_cn_avatar($matches) {
	$str = $matches[0];
	$regexp = "/(\d+|www|secure|cn|s).gravatar.com\/avatar/i";
	$str = preg_replace($regexp, 'gravatar.loli.net/avatar', $matches);
	return $str;   
}
add_filter('get_avatar', 'get_cn_avatar', 10, 3 );
...
load_theme_textdomain( 'avadaredux-framework', get_template_directory() . '/languages' );
load_theme_textdomain( 'fusion-builder', get_template_directory() . '/languages' );
/**
...
function wp_baidu_map_content_init($atts, $content = null){
    ...
    if($lon&&$lat){
        ...
        <style>            
            ...
            .mywindow{ height:auto; width:auto; font-size:14px; line-height:22px;}            
        </style>
       ...
       $output.=' var infoWindow = new BMap.InfoWindow(sContent);
		...
		var myIcon = new BMap.Icon("'.get_bloginfo('template_url') .'/assets/images/marker.gif", new BMap.Size(300,157));
